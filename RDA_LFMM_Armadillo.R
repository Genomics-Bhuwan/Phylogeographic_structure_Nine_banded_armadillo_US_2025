...
getwd()
###setwd
setwd("E:/Armadillo_Analysis/Final_Armadillo_September_3/RDA_LFMM/")
...
snps <-  vcfLink("E:/Armadillo_Analysis/Final_Armadillo_September_3/RDA_LFMM/Armadillos__filtered_within_ld_test2.vcf", overwriteID=T)
snps@meta
VCFsummary(snps) ## 115 individuals and 17025 SNPs.
## Read geno file
Dasypus_geno <- read.geno("E:/Armadillo_Analysis/Final_Armadillo_September_3/RDA_LFMM/Armadillos__filtered_within_ld_imputed.geno")
Dasypus_geno[1:10,1:10]
colnames(Dasypus_geno) <- snps@site_id
rownames(Dasypus_geno) <- snps@sample_id
dim(Dasypus_geno)

## Number of missing data
sum(is.na(Dasypus_geno))

## Load environmental data set
dados_env <- read.table("E:/Armadillo_Analysis/Final_Armadillo_September_3/RDA_LFMM/ArmadilloReferenceSamples_EnvdataRDA.csv", head=T)
head(dados_env)
str(dados_env)

## Check if individuals in genotype and environmental dataset are in the same order
data.frame(rownames(Dasypus_geno),as.character(dados_env$ID))
